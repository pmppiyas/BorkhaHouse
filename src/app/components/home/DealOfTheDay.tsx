...
const DealOfTheDay = ({ products }: DealOfTheDayProps) => {
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 md:grid-rows-2">
        ...
        {/* 3 (middle BIG) */}
        <div className="col-span-2 row-span-2 flex items-center justify-center rounded-xl border border-primary bg-card shadow-md md:col-start-2 md:row-start-1">
          {products[2] && (
            <ProductCard
              product={{
                ...products[2],
        ...
        {/* 5 (right bottom) */}
        <div className="flex items-center justify-center rounded-xl bg-card shadow-sm md:col-start-4 md:row-start-2">
          {products[4] && <ProductCard product={products[4]} />}
        </div>
      </div>
    </div>
  );
